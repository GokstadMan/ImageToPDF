...
let IE11OrLess = userAgent(/(?:Trident.*rv[ :]?11\.|msie|iemobile|Windows Phone)/i);
let Edge = userAgent(/Edge/i);
let FireFox = userAgent(/firefox/i);
let Safari = userAgent(/safari/i) && !userAgent(/chrome/i) && !userAgent(/android/i);
let IOS = userAgent(/iP(ad|od|hone)/i);
let ChromeForAndroid = userAgent(/chrome/i) && userAgent(/android/i);
...
function matrix(el, selfOnly) {
  ...
  let matrixFn = window.DOMMatrix || window.WebKitCSSMatrix || window.CSSMatrix || window.MSCSSMatrix;
  /*jshint -W056 */
